...
// In-Memory & File-backed Database Store for reliable serverless execution
...
const DB_READ_PATH = path.join(process.cwd(), 'salon-database.json');
const DB_WRITE_PATH = process.env.VERCEL
  ? path.join('/tmp', 'salon-database.json')
  : DB_READ_PATH;

function loadDatabase(): SalonDatabase {
  try {
    if (process.env.VERCEL && fs.existsSync(DB_WRITE_PATH)) {
      const raw = fs.readFileSync(DB_WRITE_PATH, 'utf-8');
      return JSON.parse(raw);
    }
    if (fs.existsSync(DB_READ_PATH)) {
      const raw = fs.readFileSync(DB_READ_PATH, 'utf-8');
      return JSON.parse(raw);
    }
  } catch (err) {
    console.warn('Could not read salon-database.json:', err);
  }
...
function saveDatabase() {
  try {
    fs.writeFileSync(DB_WRITE_PATH, JSON.stringify(db, null, 2), 'utf-8');
  } catch (err) {
    console.warn('Could not write database:', err);
  }
}
...
app.get('/api/auth/me', (req: Request, res: Response) => {
  ...
  try {
    const email = Buffer.from(token.replace('token-', ''), 'base64').toString('utf-8');
    const user = db.users.find((u) => u.email && u.email.toLowerCase() === email.toLowerCase());
...
app.get('/api/products', (req: Request, res: Response) => {
  ...
  if (inStockOnly === 'true') {
    results = results.filter((p) => (p.stock || p.stockCount || 0) > 0);
  }
  ...
  if (sort === 'price-low') {
    ...
    results.sort((a, b) => (b.reviews || b.reviewsCount || 0) - (a.reviews || a.reviewsCount || 0));
  } else if (sort === 'newest') {
    results.reverse();
  }
...
app.post('/api/products', (req: Request, res: Response) => {
  ...
  const newProduct = {
    ...
    isBestSeller: Boolean(data.bestSeller),
    tags: Array.isArray(data.tags) ? data.tags : [data.category || 'Beauty'],
  };
...
app.get('/api/categories', (_req: Request, res: Response) => {
  const productCategories = Array.from(new Set(db.products.map((p) => p.category).filter(Boolean)));
  const serviceCategories = Array.from(new Set(db.services.map((s) => s.category).filter(Boolean)));

  return res.json({
    productCategories: productCategories.length > 0 ? productCategories : ['Hair Care', 'Skin Care', 'Makeup', 'Nail Care', 'Beauty Tools', 'Body Care', 'Salon Essentials'],
    serviceCategories: serviceCategories.length > 0 ? serviceCategories : ['Hair', 'Skin', 'Makeup', 'Nails', 'Bridal', 'Beauty Care', 'Other Salon Services'],
  });
});
...
app.post('/api/orders', (req: Request, res: Response) => {
  ...
  for (const it of newOrder.items) {
    if (it && it.product && it.product.id) {
